...
class AuthService {
    async register(userData) {
        if (!isAvailable()) {
            throw new Error('Database unavailable — registration disabled');
        }
    ...
    async login(username, password, deviceInfo) {
        if (!isAvailable()) {
            throw new Error('Database unavailable — login disabled');
        }
    ...
    async searchUsers(searchTerm) {
        ...
        const result = await query(
            `SELECT id, username, display_name, avatar_url, status, last_seen
             FROM users WHERE username ILIKE $1 OR display_name ILIKE $1 LIMIT 50`,
            [`%${searchTerm}%`]
        );
        return result.rows;
    }
    ...
    async getOnlineUsers() {
        ...
        const result = await query(
            `SELECT id, username, display_name, avatar_url, status, last_seen
             FROM users WHERE status = 'online' ORDER BY display_name`
        );
        return result.rows;
    }
}
